...
import { FileText, Loader2, CheckCircle, Copy, ClipboardCopy, Download, ChevronUp, ChevronDown, Code, ScrollText } from "lucide-react";
...
export function DraftResultDisplay({ result, title = "素材整理报告" }: DraftResultDisplayProps) {
  // 添加日志查看后端返回的数据
  useEffect(() => {
    if (result) {
  ...
  useEffect(() => {
    ...
    if (typingProgress < result.content.length) {
      const timer = setTimeout(() => {
        ...
        const newProgress = Math.min(typingProgress + increment, result.content.length);
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    // 使用title作为文件名，并添加日期
    a.download = `${title}-${new Date().toLocaleDateString().replace(/\//g, '-')}.doc`;
          ...
          <div className="flex flex-col items-center gap-4">
            <div className="relative">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="h-8 w-8 rounded-full bg-blue-50 animate-ping"></div>
              </div>
            </div>
  ...
  // 计算要显示的内容（根据收起状态）
  const displayContent = isCollapsed && displayedContent.length > previewLength
        ...
        {!result.isComplete && displayedContent && !isGenerating && (
          <div className="flex items-center gap-2 mt-4 text-sm text-gray-500">
            <div className="flex gap-1">
              <span className="inline-block h-2 w-2 bg-blue-500 rounded-full animate-pulse" style={{ animationDelay: "0ms" }}></span>
