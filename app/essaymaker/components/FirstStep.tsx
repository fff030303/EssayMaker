...
export function FirstStep({
  ...
  // 添加状态来跟踪当前输入模式
  const [inputMode, setInputMode] = useState<"simple" | "draft" | "custom">("simple");
  ...
  // 添加提示文本状态
  const [placeholder, setPlaceholder] = useState<string>("你可以在这里输入问题或要求...");
  ...
  // 监听result变化，保存提纯版内容
  useEffect(() => {
    if (result && result.isComplete && inputMode === "draft" && !localIsGeneratingFinalDraft) {
      // 只有当不是在生成最终初稿时，才更新提纯版内容
      setPurifiedDraft(result.content);
      setIsPurifying(false);
      console.log("保存提纯版内容:", result.content.substring(0, 100) + "...");
    }
  }, [result, inputMode, localIsGeneratingFinalDraft]);
  ...
  const handleGenerateFinalDraft = async () => {
    ...
    try {
      ...
      if (transcriptFiles.length > 0) {
        console.log(`上传${transcriptFiles.length}个成绩单文件作为辅助资料:`, 
  ...
  const handleAdvancedFileChange = useCallback(() => {
    ...
    if (purifiedDraft) {
      ...
      console.log("FirstStep - 第一步文件变化，更新初稿文件数量:", firstStepFiles.length);
